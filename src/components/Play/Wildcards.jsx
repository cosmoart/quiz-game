		...
		<aside className='absolute top-4 right-4 lg:top-1/2 lg:-translate-y-1/2 p-2 rounded-md bg-white'>
			<nav >
				<ul className='flex gap-4 lg:flex-col'>
					...
					<li className={`relative ${wildCards.half < 1 ? "grayscale cursor-not-allowed" : "hover:scale-105"}`}>
						<button className='p-[10px] aspect-square rounded text-white bg-blue-500 w-full  transition-transform text' title='Fifty fifty' disabled={wildCards.half < 1}>
							50
						</button>
					...
					<li className={`relative ${wildCards.doubled < 1 ? "grayscale cursor-not-allowed" : "hover:scale-105"}`}>
						<button className='p-[10px] aspect-square rounded bg-blue-500 w-full transition-transform' title='Second chance' disabled={wildCards.doubled < 1}>
