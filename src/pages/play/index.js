		...
		<>
			...
			<div className='absolute left-5 top-1/2 -translate-y-1/2'>
				...
				categories: {queries.categories && queries.categories.split(",").map(category => {
					return <Image key={category} title={category} alt={category}
						...
						style={{ backgroundColor: categories.find(cat => cat.id === category).color }}
			...
			<nav className='absolute right-5 top-1/2 -translate-y-1/2'>
				<ul className='flex gap-4 flex-col'>
					{
						["Changue question", "50/50", "Second shot", "Extra life"].map((item, index) => {
							return (
								<li key={index}>
			...
			<nav>
				<ul className='flex gap-3 p-3'>
					<li>
						<Link href="/" className='w-10'><BiArrowBack color='#0f172a' className='text-3xl hover:scale-105 transition-all' title='Go back' /></Link>
					</li>
					<li>
						<button>
							<BsArrowRepeat className='text-3xl hover:scale-105 transition-all' color='#0f172a' title='New game' />
						</button>
					</li>
				</ul>
			</nav>
								...
								<div key={question.correct} className={`transition-all duration-500 ${i === 0 ? "" : "slide-right"} absolute text-center w-full`} id={"question-" + (i + 1)}>
									...
									<ul className='md:columns-2 mt-4'>
										{question.answers && question.answers.map((answer, j) => (
											<li key={j + "answer"}>
												<button className={`${"answer-" + (i + 1)} btn-primary w-full shadow-sm py-3 px-5 rounded mb-6`} onClick={(e) => validate(e, answer)}>{answer}</button>
											</li>
										))}
									</ul>
								</div>
			...
			<footer className='bg-white absolute bottom-5 right-0 left-0 mx-auto w-fit rounded py-2 px-3 flex gap-2 items-center'>
				<GoAlert className='text-2xl mx-auto' color='#0f172a' title='New game' />
				<p>
